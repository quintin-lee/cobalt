 ...
 * @details Provides a high-performance linear memory allocation mechanism, automatically handling memory alignment and on-demand reallocation.
 */
 ...
 * @details Uses a linear bump pointer approach to allocate memory, ensuring alignment for `max_align_t`.
 * If the current capacity is insufficient to accommodate the requested aligned size, it doubles the capacity.
 */
void *cobalt_arena_alloc(cobalt_arena_t *arena, size_t size)
{
    ...
    // Calculate aligned size. Uses `alignof(max_align_t)` to ensure the address is suitable for any basic data type.
    // Bitwise operations achieve rounding up for alignment.
    ...
    // The allocated address is offset from the start of the existing buffer by the number of used bytes
 ...
 * @details A very lightweight operation, just zeroes out the `used` pointer, logically freeing all allocated memory.
 */
