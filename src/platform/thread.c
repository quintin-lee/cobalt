...
{
    cobalt_thread_fn_t fn = (cobalt_thread_fn_t)(uintptr_t)arg;
    return (unsigned)(uintptr_t)fn(NULL);
}
#endif
...
#ifdef _WIN32
    HANDLE h =
        (HANDLE)(uintptr_t)_beginthreadex(NULL, 0, thread_proxy, (void *)(uintptr_t)fn, 0, NULL);
    if (!h) {
        return -1;
    }
    if (out_handle) {
