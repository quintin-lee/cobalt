 ...
 * @details Implements a hash map based on string keys. Uses the FNV-1a algorithm for hash calculation and separate chaining for hash collision resolution.
 */
 ...
 */
typedef struct {
    hashmap_node_t **buckets;      /* Bucket array, each element is a head pointer to a singly linked list */
 ...
 * @details Uses the FNV-1a (Fowler-Noll-Vo) hash algorithm, known for being simple and providing good dispersion, suitable for strings.
 * 
 ...
 * @details Allocates the initial bucket array based on initial_buckets. If the initial number of buckets is 0, allocation is deferred until the first put.
 * 
 * @param initial_buckets Capacity of the initial bucket array
 * @return Returns the newly allocated hash map, or NULL if out of memory, setting COBALT_ERROR_OUT_OF_MEMORY.
 */
cobalt_hashmap_t *cobalt_hashmap_create(size_t initial_buckets)
{
    ...
    if (initial_buckets > 0) {
        // Allocate and zero the bucket array, ensuring all linked list heads are initialized to NULL
...
/**
 * @brief Ensure the hash map has the specified minimum number of buckets, resizing and rehashing when necessary
 * @details If the current number of buckets is less than the minimum requirement, a larger bucket array is reallocated, and all existing nodes are remapped into the new buckets.
 * 
    ...
    if (impl->buckets) {
        for (size_t i = 0; i < impl->bucket_count; i++) {
            ...
            while (node) {
                ...
                unsigned int    new_idx = hash_string(node->key) % new_count; // Calculate the index in the new bucket
                
                // Insert the current node using head insertion into the linked list of the new bucket
 ...
 * @details Iterates through all buckets and linked list nodes, frees key copies and the nodes themselves, and finally frees the bucket array and the hash map structure.
 * 
 ...
 * @details If the key already exists, its corresponding value is updated. If it does not exist, a new node is allocated for insertion.
 * When the load factor exceeds 0.75, automatic rehashing (expansion) is triggered.
 * 
...
int cobalt_hashmap_put(cobalt_hashmap_t *map, const char *key, void *value)
{
    ...
    // Check load factor: (size + 1) / capacity > 0.75 is equivalent to (size + 1) * 4 / capacity > 3
    ...
    // Use head insertion to insert the new node at the beginning of the linked list for the corresponding bucket
 ...
 * @details Calculates the hash value and traverses the linked list in the corresponding bucket to find it.
 * 
 * @param map The hash map
 ...
 * @details Finds the node for the corresponding key, detaches it from the singly linked list, and frees related memory.
 * 
 * @param map The hash map
