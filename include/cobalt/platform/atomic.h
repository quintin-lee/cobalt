 ...
 * @details Encapsulates atomic operations based on C11 `stdatomic.h`, used for thread-safe counting
 * and state synchronization in multi-threaded environments.
 *
 ...
 * @note The read operation uses the `memory_order_acquire` memory order to guarantee the latest
 * value is fetched.
 */
 ...
 * @note The write operation uses the `memory_order_release` memory order, ensuring previous memory
