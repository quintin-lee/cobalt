...
#define cobalt_foreach(var, seq)                                                   \
    for (cobalt_iterator_t *_foreach_iter = cobalt_iterator_new(                   \
...
#define cobalt_foreach_rev(var, seq)                                                      \
    for (size_t _foreach_idx = cobalt_vector_size((cobalt_vector_t *)(seq));              \
         _foreach_idx > 0;                                                               \
         --_foreach_idx)                                                                 \
        for (var = cobalt_vector_get((cobalt_vector_t *)(seq), _foreach_idx - 1);        \
             var != NULL; var = (void *)0)
