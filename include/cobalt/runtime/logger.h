 ...
 * @details Provides leveled log output functionality, can output logs to a specified file stream,
 * and supports formatted strings.
 *
 ...
 * @details Defines different log levels from TRACE to FATAL, used to control the output detail
 * level.
 */
typedef enum {
 ...
 *
 * @param output_file Target file stream for log output (e.g., stdout, stderr, or a file opened via
 * fopen)
 * @param min_level   The minimum log level allowed to be output; logs below this level will be
 * discarded
 */
 ...
 * @details Filters output based on the current minimum log level, and automatically attaches
 * filename, line number, and other information.
 *
