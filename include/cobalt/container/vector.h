 ...
 * 
 * A dynamic array is a container that stores elements in a contiguous memory space, supporting fast access by index.
 * When the capacity is insufficient, it will automatically reallocate memory.
...
typedef struct {
    ...
    size_t            capacity; /**< Currently allocated memory capacity, i.e., maximum number of elements it can hold */
 ...
 * 
 * @param initial_capacity Initially allocated capacity. If 0, no initial array memory is allocated by default.
 * @return Returns a pointer to the newly created dynamic array on success; returns NULL if memory allocation fails.
 */
 ...
 * @warning This operation only frees the memory of the container itself; it does not automatically free the data pointed to by the element pointers stored in it.
 */
 ...
 * @return Returns 0 on success; returns -1 on failure (e.g., encountering a null pointer or memory allocation failure).
 */
 ...
 * @return Returns the element pointer at the specified position on success; returns NULL if vec is empty or the index is out of bounds.
 */
 ...
 * @return Returns 0 on success; if vec is empty or the index is out of bounds, returns -1 and sets the corresponding error code.
 */
 ...
 * 
 * @param vec Pointer to the target dynamic array
 * @return If the dynamic array is empty (i.e., size is 0), returns a non-zero value (1); otherwise returns 0. If vec is NULL, also returns 0.
 */
