 ...
 * @details Provides a hash map implementation based on string keys (C-strings), using separate chaining for hash collision resolution.
 */
 ...
 * @details Linked list node used to resolve collisions via separate chaining within the same bucket.
 */
struct cobalt_hashmap_node {
    const char            *key;   /**< The string key of the node */
    void                  *value; /**< Pointer to the value stored in the node */
    cobalt_hashmap_node_t *next;  /**< Pointer to the next collision node (for the collision chain) */
};

/**
 * @brief Opaque type definition for the hash map structure
 * @details Simplified implementation: currently there is no Map interface layer. The actual implementation is hidden in the .c file.
 */
typedef struct cobalt_hashmap cobalt_hashmap_t;

/**
 * @brief Create a new hash map
 * @param initial_buckets Initial number of buckets. If 0, a default size will be allocated upon lazy loading.
 ...
 * @note Will free all nodes, key copies, and the bucket array, but will not free the user data memory pointed to by value.
 */
 ...
 * @note When the load factor (size / buckets) is greater than 0.75, rehashing (expansion) will be triggered automatically.
 */
 ...
 * @return Returns the corresponding value pointer on success, or NULL if not found or an error occurs.
 ...
 * @param map Pointer to the hash map
 * @return The number of currently allocated buckets (used for debugging/verifying expansion logic), or 0 if map is NULL.
 */
