 ...
 * @details Provides a region-based linear memory allocator. It allocates a large block of memory at once and then linearly sub-allocates from it to satisfy allocation requests.
 * This greatly improves allocation speed and allows for freeing all memory at once via a reset operation, avoiding memory fragmentation and complex individual free logic.
 *
 ...
 * @note The pointer will become invalid after destruction, and all memory allocated from it will also become invalid.
 */
 ...
 * @return void* The allocated memory address, aligned to the platform's maximum alignment requirement. Returns NULL if allocation fails (e.g., out of memory and reallocation fails).
 */
 ...
 * @param arena Pointer to the arena
 * @details "Frees" all memory allocated from it at once. In reality, it zeroes out the used offset, but the underlying memory is not returned to the OS, allowing for reuse.
 */
