 ...
 * @details Provides simple in-memory JSON tree structure construction, querying, parsing (from string), and serialization (to string) capabilities.
 */
...
struct json_node {
    ...
    json_value_t      value;    /**< Specific value contained in the current node (if it's a number or string) */
    struct json_node *next;     /**< For arrays: points to the next element; for objects: points to the first key-value pair node or the next key-value pair node */
    char             *key;      /**< For object value nodes, stores its corresponding key name; NULL for arrays or root nodes */
};
 ...
 * @return Returns a dynamically allocated JSON string on success (caller must free manually), NULL or string of empty object on failure
 */
 ...
 * @return Pointer to the value node corresponding to the matching key, or NULL if not found or parent is not an object
 */
